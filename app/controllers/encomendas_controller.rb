class EncomendasController < ApplicationController
  ...
  def show
    ...
    if object != nil
      @quantidade = object.events.size

      @data = object.events.first.date
      @hora = object.events.first.hour
      @local = object.events.first.place
      @descricao = object.events.first.description
    else
      @erro = -1
    end
  ...
  private
    ...
    def encomenda_params
      params.require(:encomenda).permit(:nome, :rastreio, :email)
    end
end
